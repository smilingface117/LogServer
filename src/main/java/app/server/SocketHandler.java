...
public class SocketHandler implements Runnable {
	...
	private byte[] lineSep = AppServer.LineSep;

	public SocketHandler(SocketChannel sock, AppServer as) {
		sockCh = sock;
		srv = as;
		lineSep = System.lineSeparator().getBytes();
	}
	...
	public void run() {
		...
		try {
			while (sockCh.read(buf) >=0 || buf.position() > 0 ) {
				...
				if (len < AppServer.RECORD_SIZE && len >= lineSep.length ) {
					//check if we got shorter line
					int pos = len;
					for (int j = 0; j < lineSep.length; j++) {
						if (lineSep[lineSep.length - 1 - j] != buf.get(pos - 1
								- j)) {
							pos = -1;
							break;
						}
					...
					int pos = (len / AppServer.RECORD_SIZE) * AppServer.RECORD_SIZE;
